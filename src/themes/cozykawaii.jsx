...
// Read the fullscreen-tip flag ONCE at module load (not per-render) so a
// re-render can't re-trigger the hint. Mirrors INITIAL_EXISTING_USER pattern.
const INITIAL_FS_TIP_SEEN = (() => {
  try { return localStorage.getItem('cozydesk_fullscreen_tip_seen') === 'yes'; }
  catch (_) { return true; }
})();
...
export default function Cozykawaii() {
  ...
  const [showFsTip, setShowFsTip] = useState(!INITIAL_FS_TIP_SEEN);

  // Keep the button icon in sync with the browser's real fullscreen state.
  // Sync immediately on mount too (not just on change events) so a world-switch
  // remount can't leave the icon showing a stale state — the same "don't trust
  // the mount-time snapshot" trap as INITIAL_EXISTING_USER.
  useEffect(() => {
  ...
  const dismissFsTip = useCallback(() => {
    try { localStorage.setItem('cozydesk_fullscreen_tip_seen', 'yes'); } catch (_) {}
    setShowFsTip(false);
  }, []);

  // Fullscreen can ONLY be triggered by a user gesture (browser rule). Guarded
  // so a Safari/Firefox rejection fails silently instead of crashing the app.
  const toggleFullscreen = useCallback(() => {
    ...
    } catch (_) { /* fullscreen unsupported/blocked — ignore */ }
  }, []);
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          background: 'linear-gradient(180deg, rgba(255,255,255,0.55) 0%, rgba(255,255,255,0.12) 45%, rgba(255,255,255,0.06) 100%), rgba(255,255,255,0.30)',
      ...
      >
        <svg width="22" height="22" viewBox="0 0 20 20" fill="none"
          stroke="#4b3b2a" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
          {isFullscreen ? (
            <>
              <path d="M1 5 L5 5 L5 1 M5 5 L1 1" />
              <path d="M19 15 L15 15 L15 19 M15 15 L19 19" />
            </>
          ) : (
            <>
              <path d="M5 1 L1 1 L1 5 M1 1 L5 5" />
              <path d="M15 19 L19 19 L19 15 M19 19 L15 15" />
            </>
          )}
        </svg>
      </button>

      {/* ── One-time "go fullscreen" cozy tip ── */}
      {showFsTip && (
        <div
          onClick={e => e.stopPropagation()}
          style={{
            position: 'absolute', top: '62px', right: '16px', zIndex: 400,
            maxWidth: '260px',
            background: 'rgba(255,253,248,0.96)', backdropFilter: 'blur(5px)',
            border: '1px solid rgba(224,212,200,0.7)', borderRadius: '14px',
            boxShadow: '0 6px 24px rgba(0,0,0,0.14)',
            padding: '14px 16px', fontFamily: "'Nunito', sans-serif",
          }}
        >
          <p style={{ margin: '0 0 12px 0', color: '#4b3b2a', fontSize: '0.88rem', fontWeight: 600, lineHeight: 1.4 }}>
            🌸 For the best fit, go fullscreen before you decorate. Your desk arranges itself to the window size, so setting it up big keeps everything where you want it.
          </p>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <button
              onClick={() => { toggleFullscreen(); dismissFsTip(); }}
              style={{ flex: 1, padding: '8px 10px', border: 'none', borderRadius: '10px', background: '#d4a373', color: 'white', fontFamily: "'Nunito', sans-serif", fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer' }}
            >
              Go fullscreen ✨
            </button>
            <button
              onClick={dismissFsTip}
              style={{ padding: '8px 10px', border: 'none', borderRadius: '10px', background: '#eee', color: '#4b3b2a', fontFamily: "'Nunito', sans-serif", fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer' }}
            >
              Maybe later
            </button>
          </div>
