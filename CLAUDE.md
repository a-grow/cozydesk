...
Last updated: Aug 10, 2026 (Gumroad products setup complete). Read fully before touching any code.
...
2. NEXT: Build + verify the license-key unlock in the app. Plan-in-chat first (start recon with themeRegistry.js to see how worlds + free/paid are structured), then Claude Code with HARD RULES. Approach decided: live Gumroad API check on key entry → cache unlock in localStorage → works offline after (never re-check every launch). Locked worlds show a clean preview screen (NOT grayed-out-tools). VERIFY the full loop end-to-end (real/test purchase → email key → paste → unlock → offline after) before relying on it.
