...
NEXT SESSION — ordered plan: (1) Lo-Fi timer-reset fix (structural, high priority, full budget). (2) Double-click-to-add stickers (tester request). (3) Move timer to top of Productivity Tools (both sidebars, unchanged styling). (4) First-focus reveal moment (cozydesk_focus_revealed flag). (5) Install-to-dock button (both sidebars, feature-detected). (6) New stickers for other worlds (art-only; cheapest — good low-budget filler). (7) BLOCKED — "morning three things" (needs to-do date-stamp = save-slot data-shape change; do last, after beta).
...
## KNOWN BUG (diagnosed, still UNFIXED as of Aug 5) — HIGH PRIORITY / #1 NEXT SESSION — Pomodoro timer resets across Lo-Fi
