...
Last updated: Aug 3, 2026. Read fully before touching any code.
...
NEXT SESSION — ordered plan: (1) MERGE experiment-focus → dev (focus mode goes live) — backup, merge, test every world + focus flow on localhost, bump package.json version, then push → auto-deploys; this is a PUSH TO LIVE, do fresh. (2) Lo-Fi timer-reset fix (structural). (3) Double-click-to-add stickers + tool stickers (tester request). (4) New stickers for existing worlds (art-only, after merge). (5) PARKED/BLOCKED — "morning three things" opening question (retention hook); needs a date-stamp on to-do items = save-slot data-shape change that must NOT land mid-beta; do last, after beta.
...
## Experiment Branches (as of Aug 3, 2026 — NOT on dev, NOT shipped)
Two experiment branches off `dev`. Neither merged yet; the MERGE is the first task next session. `dev` and the live app are untouched by either.
- `experiment-mascot` (`c2a672a`) — "Mugzy" mascot base: bottom-right 132px badge, idle breathing, sparkle-pop on task complete, Settings toggle (localStorage `cozydesk_mugzy`), event bus (`src/utils/mugzyBus.js`), poses in `src/assets/mascot/`.
- `experiment-focus` (HEAD `21c3b2e`, off experiment-mascot) — Focus Mode + ALL Aug 2–3 polish; this is the branch to merge. Commits (newest first): `21c3b2e` focus glow (circle + card frame); `1303547` Mugzy toggle button (3D yellow, summon-on-start, click sound); `1512b2f` Pomodoro buttons Patrick Hand→Nunito; `3644aaf` to-do fixes (commit-on-blur, remove hover-glow, Safari drag fix); `e890605` focus polish (sticker settle-bounce, music pause on stop, Mugzy blink+glow); `8c290b6` CLAUDE.md notes; `b255b79` original focus mode.
...
## KNOWN BUG (diagnosed, still UNFIXED as of Aug 3) — Pomodoro timer resets across Lo-Fi
Timer survives switching among kawaii↔steampunk↔café (shared `<div className="cozy-sidebar">` in `Sidebar.jsx`) but RESETS to 25:00 crossing into/out of Lo-Fi, because `Sidebar.jsx` returns `<LofiSidebar/>` for Lo-Fi — an element-TYPE change unmounts PomodoroTimer and wipes its local `useState`. Re-confirmed Aug 3; now high-priority since focus mode makes the timer central. Fix (NOT built): move timer state (`timeLeft`,`isRunning`,`mode`) OUT into shared/persisted state so both sidebars read it and it survives the remount. Structural — do fresh, test all world-switch combos. Second task next session (after the merge).
