...
Last updated: Aug 11, 2026 (World Gallery Step 2 + Pomodoro redesign shipped). Read fully before touching any code.
...
## License Unlock Feature — IN PROGRESS
Locked-world UX (DECIDED): clicking a locked/paid world shows a clean PREVIEW SCREEN (full-bleed world bg + card with name, flavor text, Unlock→Gumroad, "already bought? enter key" link) — NOT the "grayed-out tools inside the desk" version. Onboarding first screen shows only the 2 FREE worlds; paid worlds discovered via the gallery.

- **Step 1 — DONE.** `src/utils/licenseManager.js`: owns paid/free map (cafe+steampunk paid; cozykawaii+lofi free), product IDs, `verifyAndUnlock(world, key)` (checks key against Gumroad's verify endpoint — own product then bundle — validates not refunded/chargebacked/disputed, caches to localStorage `cozydesk_unlocked`, offline forever after). Exposes `isPaidWorld()`, `isWorldUnlocked()`. Gumroad verify confirmed secret-free (product_id + license_key only); passes `increment_uses_count:false`. Console-tested: paid/free logic correct.
- **Step 2 — DONE.** World Gallery replaces the old native `<select>`. `src/components/sidebar/WorldGalleryModal.jsx` (renders via `createPortal` to `document.body` to escape the sidebar's overflow clip). `ThemesSection.jsx` (shared by both sidebars) shows a "Choose Your World" trigger → gallery. Gallery: 4 world cards (cropped backgrounds), lock badge + $2.99 on paid worlds, click-locked → preview screen (Unlock opens Gumroad; "Already bought?" link is inert — console.logs — until Step 3). "Themes"→"Worlds" label in both sidebars (label only; full rename still parked). Reads `isPaidWorld`/`isWorldUnlocked` — no Gumroad calls yet. Gallery-open plays `sfx_click_button`; world switch plays `sfx_place_note`.
- **OPEN UNKNOWN before Step 3 — CORS.** Whether the browser can reach Gumroad's `POST https://api.gumroad.com/v2/licenses/verify` without a CORS error is UNTESTED (their docs only show curl/server-side). Needs a REAL purchased key: do a $2.99 Café test-buy, then a console `fetch` test. If CORS blocks → fallback is a tiny free proxy function (does NOT change licenseManager logic). Resolve this BEFORE building Step 3 so we don't build the modal twice.
- **Step 3 — NEXT.** Enter-key modal: wire the "Already bought?" link → key input → `verifyAndUnlock` → unlock + close.
- **Step 4.** Gate `setTheme`/`setThemeDirect` against locked worlds + onboarding shows free worlds only.
- **Step 5.** End-to-end test (buy → email → paste key → unlock → offline persists).

## Landing Page (LIVE)
Fully redesigned and live (repo `a-grow/cozydesk-landing`): live-app framing ("Open CozyDesk"), transparent logo, all 4 worlds with correct free/paid tags, dark worlds-band, $10.99 bundle CTA, soft/non-binding "$100 → 2 new worlds" note, browser-aware install hint, footer legal links + cozydesksupport@gmail.com. Still open (non-blocking): Lo-Fi hero video (hero.mp4 in images/); confirm the bundle CTA points at the real `/l/all-access` URL (was a GUMROAD_BUNDLE_URL placeholder). Claude cannot edit/deploy the landing repo — Andrew runs Claude Code there separately.

## Launch Prep — Status
Shipped & LIVE: cache fix, error boundary, backup/restore, Settings version + Report a Bug, legal docs, onboarding, deploy, smoke test, focus mode + Mugzy, timer-reset fix, double-click-to-add, press-Start nudge, landing redesign, Gumroad products, World Gallery (Step 2), Pomodoro redesign. Friends beta DONE; beta push freeze LIFTED.
Pre-public-launch blockers still open: (1) small-screen notice for phone visitors; (2) finish the license unlock feature (Steps 3–5); (3) landing hero video + bundle URL confirm; (4) cover images for the Gumroad products.

## Completed — Do Not Rebuild or Re-break (one-line index)
Universal sticky notes; per-theme maxItems + delete-completed ✕; settings panel zIndex 600; pin-to-front + Unpin; clock flip + XS–XL sizing; attach/detach (stickers + notes); Save / My Desks (10 slots/theme); lofi sidebar icons; steampunk gears + mahogany sidebar; per-theme theme-color meta; aspect-ratio locking; 4 tracks/theme; sticker box hugs art; storage-full warning; calendar sharing; rotation persistence (rotationRef pattern: onMove writes ref, onUp reads it — dodges stale closure; add fns seed rotation:0; initialRotation prop seeds on load); cache/app-shell fix; error boundary; backup/restore; cross-theme save-overwrite fix; Settings live version + Report-a-Bug mailto; SavePopup dark-mode input fix; Focus Mode + Mugzy (v1.2.0); onboarding existing-user freeze fix; new café stickers; Lo-Fi timer-reset fix (timerStore singleton); double-click-to-add; press-Start nudge; Lo-Fi sleeping-dog sticker; World Gallery Step 2; Pomodoro redesign (candy button + Nunito + lofi fixes) + gallery/pause sounds.
...
- Small-screen notice: warm "CozyDesk is built for a bigger screen" message for phone visitors. Needed before public launch.
